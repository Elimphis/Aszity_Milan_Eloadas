import { ListaElem } from "./ListaElem.js";

export class Feladat extends ListaElem {


    constructor(szoveg, kesz = false) {
    ...
    megjelenit(torlesCallback, frissitesCallback) {

        let elem = super.megjelenit();
        elem.classList.add("feladat");

        let szovegElem          = document.createElement("span");
        szovegElem.textContent  = this.szoveg;

        if (this.kesz) {
            szovegElem.classList.add("kesz");
        }
        ...
        torlesGomb.classList = 'flex padding-1 border-soft bg-none'
        torlesGomb.innerHTML = `
        ...
        gombok.appendChild(keszGomb);
        gombok.appendChild(torlesGomb);

        elem.appendChild(szovegElem);
        elem.appendChild(gombok);
